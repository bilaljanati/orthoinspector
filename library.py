	...
	def get_species_list(self):
		if self.species_list == None:
			...
			for species in liste:
				self.species_list[species["taxid"]] = Species(self, species["taxid"], species["name"])
...
class Species: #passer par un objet bdd
	...
	def get_all_proteins(self):
		if self.all_proteins == None:
			...
			for prot in data:
				self.all_proteins[prot["access"]] = Protein(self, prot["access"], prot["name"])
	...
	def get_orthologs(self, taxid2 = None):
		...
		if self.taxid2 != None:
			...
			for orth in list_orthologs:
				...
				if orth["type"] == "One-to-one":
					...
					self.orthologs.append(Orthologs(sp1, taxid2, pr1, access2, inparalogs1, inparalogs2, orth["type"]))
		...
		if self.taxid2 == None:
			...
			for item in list_orthologs:
				...
				access2 = inparalogs2[0]
				#pr2 = sp2.get_protein(access2)
				orthologtemp = Ortholog(sp1, taxid2, pr1, access2, inparalogs1, inparalogs2, item["type"])
