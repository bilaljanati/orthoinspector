...
def home():
    return render_template(
        "index.html", dblist=wh.get_dblist(), stats=wh.gather_stats()
    )


# does not work http://localhost:5000/oi/Archaea/2023
...
# works http://localhost:5000/oi/Archaea/2023/species
...
# works http://localhost:5000/oi/Archaea/2023/tree/sun
@bp.route("/api/<database>/<int:release>/tree/sun")
...
# works http://localhost:5000/oi/Archaea/2023/tree/profile
...
# works http://localhost:5000/oi/Eukaryota/2023/search/protein?term=MTH
...
def protein(database, release, access):
    ...
    if not db.has_models and model_only:
        return redirect(
            url_for("bp.protein", database=database, release=release, access=access),
            code=302,
        )
...
# Doesn't work with dbname but with database yes: http://localhost:5000/oi/Eukaryota/2016/orthologs/P83120, not OrthoInspector_Eukaryota_2023 (as contained in config.yml)
...
# works http://localhost:5000/oi/Eukaryota/2023/protein/Q6KAQ7
...
# works http://localhost:5000/oi/Eukaryota/2023/distribution/Q6KAQ7
...
# works http://localhost:5000/oi/annotations/go/Q6KAQ7
...
def blast_search_run():
    res = submit_task(
        config["worker_pool"]["host"], "blast_search", request.form.to_dict()
    )
    return jsonify(res)
...
def go_search_run():
    ...
    for key in ["database", "release", "taxid", "goid", "species_name", "goname"]:
        params[key] = request.form[key]
    job_params = {
        k: v for k, v in params.items() if k not in ["species_name", "goname"]
    }
    res = submit_task(config["worker_pool"]["host"], "go_search", job_params)
    return render_template(
        "gosearchresult.html", params=params, taskid=res["id"], dblist=wh.get_dblist()
    )
...
def data():
    return render_template(
        "data.html",
        dblist=wh.get_dblist(),
        base_url=config["warehouse"]["data_server_url"],
    )
...
def db_data(database, release):
    ...
    if not db:
        abort(404)
    data = {"species": db.get_species_list()}
    return jsonify(data)
...
def db_list():
    return render_template(
        "dblist.html", dblist=wh.get_dblist(), dbstats=wh.gather_stats()
    )
