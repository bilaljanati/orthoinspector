 ...
 */
//Yannis Nevers mod: for a tristate checkbox support we need to replace selected class by two classes: selectedPres, selectedAbs
...
var i, attr,
	...
	//YN replaced selected by selectedAbs and selectedPres
	CLASS_ATTRS = "active expanded focus folder lazy radiogroup selectedPres selectedAbs unselectable unselectableIgnore".split(" "),
	CLASS_ATTR_MAP = {},
	...
	//YN replaced selected by selectedAbs and selectedPres
	NODE_ATTRS = "checkbox expanded extraClasses folder icon iconTooltip key lazy partsel radiogroup refKey selectedPres selectedAbs statusNodeType title tooltip type unselectable unselectableIgnore unselectableStatus".split(" "),
	NODE_ATTR_MAP = {},
 ...
 * @property {boolean} selectedPres Use isSelectedPres(), setSelectedPres() to access this property. Mod YN.
 * @property {boolean} selectedPres Use isSelecteAbs(), setSelectedAbs() to access this property. Mod YN.
...
function FancytreeNode(parent, obj){
	...
	//YN modified. Changed selected to selectedAbs and selectedPres
	if( obj.selectedAbs || obj.selectedPres ){ // #186
...
FancytreeNode.prototype = /** @lends FancytreeNode# */{
	...
	*/
	...
	/*YN: modified to allow for different selected state (Selected Abs and SelectedPres)
	...
	_changeSelectStatusAttrs: function(state) {
		...
		switch(state){
		case false:
			changed = ( this.selectedAbs || this.selectedPres || this.partsel );
			this.selectedAbs = false;
			this.selectedPres = false;
			this.partsel = false;
			break;
		case 'Present':
			changed = ( !this.selectedPres || !this.partsel );
			this.selectedPres = true;
			this.selectedAbs = false;
			this.partsel = false;
			break;
		case 'Absent':
			changed = ( !this.selectedAbs || !this.partsel );
			this.selectedPres = false;
			this.selectedAbs = true;
			this.partsel = false;
			break;
		case undefined:
			changed = ( this.selectedAbs || this.selectedPres || !this.partsel );
	 ...
	 */
	 /** YN mod: all children are selected only when absent is selected*/
	fixSelection3AfterClick: function(callOpts) {
		var present = this.isPresent();
		var absent = this.isAbsent();
		if (!present) {
			this.visit(function(node){
			node._changeSelectStatusAttrs(absent ? 'Absent' : false);
		});
...
//		this.debug("fixSelection3AfterClick()");
	 ...
	 */
	 //YN TODO: Revisit when more is understood
...
//		this.debug("fixSelection3FromEndNodes()");
		...
		function _walk(node){
			...
			if( children && children.length ){
				...
				state = allSelected ? 'Absent' : (someSelected ? undefined : false);
			}else{
				// This is an end-node: simply report the status
				unselState = FT.evalOption("unselectableStatus", node, node, opts, undefined);
				state = ( unselState == null ) ? (!!node.selectedPres || !!node.selectedAbs) : !!unselState;
			}
			node._changeSelectStatusAttrs(state ? (node.selectedPres ? 'Present' : 'Absent') : state);
			return state;
		}
		if (this.isAbsent()) {
		_walk(this);
		...
		this.visitParents(function(node){
			...
			for( i=0, l=children.length; i<l; i++ ){
				...
				//console.log(child.title+ ":"+ child.selectedPres+":"+child.selectedAbs);

				unselIgnore = FT.evalOption("unselectableIgnore", child, child, opts, false);
				if( !unselIgnore ) {
					unselState = FT.evalOption("unselectableStatus", child,  child, opts, undefined);
					state = ( unselState == null ) ? !!child.selectedAbs : !!unselState;
			...
			state = allAbsent ? 'Absent' : (someAbsent ? undefined : false);
			//console.log(state);
...
//		return this.parent.children.indexOf(this);
	...
	/**
	 * Return an array of descendant nodes with attribute present.
	 * @param {boolean} [stopOnParents=false] only return the topmost present nod
	 *     node (useful with selectMode 3)
	 * @returns {FancytreeNode[]}
	 */
	 /** YN mod: get selected Pres nodes*/
	getPresentNodes: function(stopOnParents) {
		var nodeList = [];
		this.visit(function(node){
			if( node.selectedPres ) {
				nodeList.push(node);
		...
		/**
	 * Return an array of absent descendant nodes.
	 * @param {boolean} [stopOnParents=false] only return the topmost absent node
	 *     node (useful with selectMode 3)
	 * @returns {FancytreeNode[]}
	 */
	 	 /** YN mod: get selectedAbs nodes*/

	getAbsentNodes: function(stopOnParents) {
		var nodeList = [];
		this.visit(function(node){
			if( node.selectedAbs ) {
				nodeList.push(node);
	 ...
	 * @since 2.23
	 */
	 /**YN mod: check for Pres and Abs rather than selected*/
	isPartsel: function() {
		return !this.selectedAbs && !this.selectedPres && !!this.partsel;
	},
	...
	/** Return true if node is selectedPres, i.e. has a checkmark set (see also FancytreeNode#isActive).
	 * @returns {boolean}
	 */
	/** YN mod: now work for 'present' selection */
	isPresent: function() {
		return !!this.selectedPres;
	},
		/** Return true if node is selectedAbs, i.e. has a cross set (see also FancytreeNode#isActive).
	 * @returns {boolean}
	 */
	 /** YN mod: now work for 'absent' selection */
	isAbsent: function() {
		return !!this.selectedAbs;
	},
...
//		_assert($(this.span).is(":visible"), "scrollIntoView node is invisible"); // otherwise we cannot calc offsets
	...
	/**Select this node as present, i.e. check the checkbox.
	 ...
	 */
	 /** YN mod: change selected to selectedPres*/
	setPresent: function(flag, opts){
		return this.tree._callHook("nodeSetPresent", this, flag, opts);
	},
	/**Select this node as absent, i.e. cross the checkbox.
	 * @param {boolean} [flag=true] pass false to deselect
	 * @param {object} [opts] additional options. Defaults to {noEvents: false, p
	 *     propagateDown: null, propagateUp: null, callback: null }
	 */
	 /** YN mod: change selected to selectedAbs*/
	setAbsent: function(flag, opts){
		return this.tree._callHook("nodeSetAbsent", this, flag, opts);
	},
...
//		this.debug("_hook", funcName, ctx.node && ctx.node.toString() || ctx.tree.toString(), args);
   ...
   */
	...
	*/
	 ...
	 */
	 //YN: modified to get both absent and present elements. Needs revisiting for filter.
	generateFormElements: function(selected, active, opts) {
		opts = opts || {};

		var nodeList,
			presentName = (typeof selected === "string") ? selected : "ft_pres_" + this._id + "[]",
			absentName = (typeof selected === "string") ? selected : "ft_absent_" + this._id + "[]",
		...
		if(active !== false && this.activeNode){
			$result.append($("<input>", {
				type: "radio",
				name: activeName,
				value: this.activeNode.title,
				checked: true
			}));
		}
		function _appender( node, present ) {
			if (present) {
			$result.append($("<input>", {
				type: "checkbox",
				name: presentName,
		...
		if ( opts.filter ) {
			this.visit(function(node) {
				...
				if ( res !== false ) {
					//TODO: Revisit that (YN)
					_appender(node, true);
				}
			});
		} else if ( this.selectedPres !== false || this.selectedAbs !== false) {
			var nodePres = this.getPresentNodes(false);
			$.each(nodePres, function(idx, node) {
				_appender(node, true);
			});
			var nodeAbs = this.getAbsentNodes(stopOnParents);
			$.each(nodeAbs,function(idx, node) {
				_appender(node, false);
			});
		}
	},
	/**
	...
	/**
	 * Return an array of present nodes.
	 * @param {boolean} [stopOnParents=false] only return the topmost selected
	 *     node (useful with selectMode 3)
	 * @returns {FancytreeNode[]}
	 */
	 //YN: modified to get present nodes
	getPresentNodes: function(stopOnParents) {
		return this.rootNode.getPresentNodes(stopOnParents);
	},
		/**
	 * Return an array of absent nodes.
	 * @param {boolean} [stopOnParents=false] only return the topmost selected
	 *     node (useful with selectMode 3)
	 * @returns {FancytreeNode[]}
	 */
	 //YN: modified to get absent nodes
	getAbsentNodes: function(stopOnParents) {
		return this.rootNode.getAbsentNodes(stopOnParents);
	},
...
*/
	...
	/**(De)select all nodes as present.
	 * @param {boolean} [flag=true]
	 * @since 2.28
	 */
	 //Mod YN: change to selected to present
	selectPresAll: function(flag) {
		this.visit(function(node){
			node.setPresent(flag);
		});
	},
		/**(De)select all nodes as present.
	 * @param {boolean} [flag=true]
	 * @since 2.28
	 */
	//Mod YN: change to selected to absent
	selectAbsAll: function(flag) {
		this.visit(function(node){
			node.setAbsent(flag);
		});
	},
...
//			tree.$container.attr("tabindex", "0");
		...
		//YN Mod for present and absent rather than select
		if( node.selectedPres ){
			cnList.push(cn.selectedPres);
			if(aria){
				$ariaElem.attr("aria-selected-present", true);
			}
		} else if (node.selectedAbs)
		{
			cnList.push(cn.selectedAbs);
			if(aria){
				$ariaElem.attr("aria-selected-absent", true);
			}
		}
		else if(aria){
			$ariaElem.attr("aria-selected-present", false);
			$ariaElem.attr("aria-selected-absent", false);
		}
...
//          }
	...
	/** (De)Select node as present, return new status (sync).
	 *
	 * @param {EventData} ctx
	 * @param {boolean} [flag=true]
	 ...
	 * @returns {boolean} previous status
	 */
	 //YN. Mod to adapt to present/absent
	nodeSetPresent: function(ctx, flag, callOpts) {
		callOpts = callOpts || {};
		var node = ctx.node,
			tree = ctx.tree,
			opts = ctx.options,
		...
		node._lastSelectIntent = flag ? 'Present' : false;

		// Nothing to do?
		/*jshint -W018 */  // Confusing use of '!'
		if( !!node.selectedPres === flag ){
		...
		if( !noEvents &&
			this._triggerNodeEvent("beforeSelect", node, ctx.originalEvent) === false ) {
				return !!node.selectedPres;
		}
		if(flag && opts.selectMode === 1){
			// single selection mode (we don't uncheck all tree nodes, for performance reasons)
			if(tree.lastSelectedNode){
				tree.lastSelectedNode.setSelectedPres(false);
			}
			node.selectedPres = flag;
			...
			node.visitSiblings(function(n){
				n._changeSelectStatusAttrs(flag && n === node ? 'Present' : false);
			}, true);
		}else{
	...
	/** (De)Select node as absent, return new status (sync).
	 *
	 * @param {EventData} ctx
	 * @param {boolean} [flag=true]
	 * @param {object} [opts] additional options. Defaults to {noEvents: false,
	 ...
	 */
	 //YN. Mod to adapt to present/absent
	nodeSetAbsent: function(ctx, flag, callOpts) {
		callOpts = callOpts || {};
		var node = ctx.node,
			tree = ctx.tree,
			opts = ctx.options,
		...
		// Remember the user's intent, in case down -> up propagation prevents
		// applying it to node.selected
		node._lastSelectIntent = flag ? 'Absent' : false;

		// Nothing to do?
		/*jshint -W018 */  // Confusing use of '!'
		if( !!node.selectedAbs === flag ){
			if( opts.selectMode === 3 && node.partsel && !flag ){
				// If propagation prevented selecting this node last time, we still
				// want to allow to apply setSelected(false) now
			}else{
		...
		if( !noEvents &&
			this._triggerNodeEvent("beforeSelect", node, ctx.originalEvent) === false ) {
				return !!node.selectedAbs;
		}
		if(flag && opts.selectMode === 1){
			// single selection mode (we don't uncheck all tree nodes, for performance reasons)
			if(tree.lastSelectedNode){
				tree.lastSelectedNode.setSelectedAbs(false);
			}
			node.selectedAbs = flag;
		}else if(opts.selectMode === 3 && parent && !parent.radiogroup && !node.radiogroup){
			// multi-hierarchical selection mode
			node.selectedAbs = flag;
			node.fixSelection3AfterClick(callOpts);
			...
			node.visitSiblings(function(n){
				n._changeSelectStatusAttrs(flag && n === node ? 'Absent' : false);
			}, true);
		}else{
			// default: selectMode: 2, multi selection mode
			node.selectedAbs = flag;
	 ...
	 * @param {EventData} ctx
	 */
	 //YN: Rewritten to handle present/absent paradigm
	nodeToggleSelected: function(ctx) {
		var node = ctx.node;

		if (!node.selectedPres && !node.selectedAbs)  {
			...
			if( node.partsel  && !!node._lastSelectIntent === true ) {
				node.selectedPres = true;  // so it is not considered 'nothing to do'
				node.selectedAbs = true;
				this.nodeSetPresent(ctx, false);
				return this.nodeSetAbsent(ctx, false);
			} else {
				node._lastSelectIntent = 'Present';
				return this.nodeSetPresent(ctx, true);
			}
		} else if (node.selectedPres) {
			node._lastSelectIntent = 'Absent';
			this.nodeSetPresent(ctx, false);
			return this.nodeSetAbsent(ctx, true);
		} else if (node.selectedAbs) {
			node._lastSelectIntent = false;
			return this.nodeSetAbsent(ctx, false);
		}
		return false;
	},
...
//          timeout: 0, // >0: Make sure we get an ajax error if server is unreachable
		...
		_classNames: {
			...
			selectedPres: "fancytree-selected-present",
			selectedAbs: "fancytree-selected-absent",
...
$.ui.fancytree._FancytreeClass.prototype.countSelected = function(topOnly){
	...
	count = tree.getPresentNodes(true).length+tree.getAbsentNodes(false).length;
	return count;
};
...
//						dropEffect: undefined,  // set by dragend
				...
				case "dragstart":
					...
					if( dndOpts.multiSource === false ) {
						...
						SOURCE_NODE_LIST = tree.getPresentNodes();
						if( !node.isPresent() ) {
 ...
 * Private functions and variables
 */
//Mod YN: adapted icon to presence/absence
var FT = $.ui.fancytree,
	PRESETS = {
		"awesome3": {  // Outdated!
			_addClass: "",
		...
		"awesome4": {
			...
			checkboxPresent: "fa-check-square-o",
			checkboxAbsent: "fa-square",
			...
			radioPresent: "fa-circle",
			radioAbsent:"fa-square",
		...
		"awesome5": {
			...
			checkboxPresent: "far fa-check-square",
			checkboxAbsent: "fas fa-check-square",
			// checkboxUnknown: "far fa-window-close",
			checkboxUnknown: "fas fa-square fancytree-helper-indeterminate-cb",
			radio: "far fa-circle",
			radioPresent: "fas fa-circle",
			radioAbsent: "fas fa-check-square",
		...
		"bootstrap3": {
			...
			checkboxPresent: "glyphicon-check",
			checkboxAbsent: "glyphicon-remove",
		...
		"material": {
			...
			checkboxPresent: { text: "check_box" },
			checkboxAbsent: {text: "cancel"},
			...
			radioPresent: { text: "radio_button_checked" },
			radioAbsent:  {text: "cancel"},
			// Default node icons.
			// (Use tree.options.icon callback to define custom icons based on node data)
...
$.ui.fancytree.registerExtension({
	...
	nodeRenderStatus: function(ctx) {
		...
		//Mod YN: adapt rendering to presence/absence
		if( span ) {
			checkbox = FT.evalOption("checkbox", node, node, opts, false);
 ...
 * Private functions and variables
 */
 //YN mod: changed select to presence/absence
...
$.ui.fancytree._FancytreeClass.prototype.clearPersistData = function(types){
	...
	types = types || "active expanded focus selectedPres selectedAbs";
	...
	if(types.indexOf(SELECTEDPRES) >= 0){
		local._data(prefix + SELECTEDPRES, null);
	}
	if(types.indexOf(SELECTEDABS) >= 0){
		local._data(prefix + SELECTEDABS, null);
	}
};
 ...
 * @requires jquery.fancytree.persist.js
 */
 //YN mod: change select to presence/absence
$.ui.fancytree._FancytreeClass.prototype.getPersistData = function(){
	...
	res[SELECTEDPRES] = (local._data(prefix + SELECTEDPRES) || "").split(delim);
	res[SELECTEDABS] = (local._data(prefix + SELECTEDABS) || "").split(delim);
 ...
 * Extension code
 */
 //Mod YN: changed select to presence/absence
$.ui.fancytree.registerExtension({
	name: "persist",
	version: "2.28.1",
	// Default options for this extension.
	options: {
		...
		types: "active expanded focus selectedPres selectedAbs"
	},
	...
	//Mod YN: change select to presence/absence
	treeInit: function(ctx){
		var tree = ctx.tree,
			opts = ctx.options,
			local = this._local,
		...
		local.storeSelectedPres = instOpts.types.indexOf(SELECTEDPRES) >= 0;
		local.storeSelectedAbs = instOpts.types.indexOf(SELECTEDABS) >= 0;
		...
		tree.$div.on("fancytreeinit", function(event){
			...
			//Mod YN: change selected to presene/absence
			dfd.done(function(){
				if(local.storeSelectedPres){
					cookie = local._data(local.cookiePrefix + SELECTEDPRES);
					if(cookie){
						keyList = cookie.split(instOpts.cookieDelimiter);
						for(i=0; i<keyList.length; i++){
							node = tree.getNodeByKey(keyList[i]);
							if(node){
								if(node.selectedPres === undefined || instOpts.overrideSource && (node.selectedPres === false)){
//									node.setSelected();
									node.selectedPres = true;
									node.renderStatus();
								}
							}else{
								// node is no longer member of the tree: remove from cookie also
								local._appendKey(SELECTEDPRES, keyList[i], false);
							}
						}
					}
				...
				if(local.storeSelectedAbs){
						cookie = local._data(local.cookiePrefix + SELECTEDABS);
						if(cookie){
							keyList = cookie.split(instOpts.cookieDelimiter);
							for(i=0; i<keyList.length; i++){
								node = tree.getNodeByKey(keyList[i]);
								if(node){
									if(node.selectedAbs === undefined || instOpts.overrideSource && (node.selectedAbs === false)){
	//									node.setSelected();
										node.selectedAbs = true;
										node.renderStatus();
									}
									...
									// node is no longer member of the tree: remove from cookie also
									local._appendKey(SELECTEDABS, keyList[i], false);
								}
							}
						}
						// In selectMode 3 we have to fix the child nodes, since we
	...
	//YN mod: select to presence/absence
	nodeSetSelectedPres: function(ctx, flag, callOpts) {
		var res, selNodes,
			tree = ctx.tree,
			node = ctx.node,
			local = this._local;
		...
		if(local.storeSelectedPres){
			if( tree.options.selectMode === 3 ){
				...
				presNodes = $.map(tree.getPresNodes(false), function(n){
					return n.key;
				});
				presNodes = presNodes.join(ctx.options.persist.cookieDelimiter);
				local._data(local.cookiePrefix + SELECTEDPRES, presNodes);
			} else {
				// beforeSelect can prevent the change - flag doesn't reflect the node.selected state
				local._appendKey(SELECTEDPRES, node.key, node.selectedPres);
			}
		}
		return res;
	},
	//YN mod: select to presence/absence
	nodeSetSelectedAbs: function(ctx, flag, callOpts) {
		var res, selNodes,
			tree = ctx.tree,
			node = ctx.node,
			local = this._local;
		...
		if(local.storeSelectedAbs){
			if( tree.options.selectMode === 3 ){
				...
				// De-selecting a node may also de-select some parents, so we
				// calculate the current status again
				absNodes = $.map(tree.getAbsNodes(true), function(n){
					return n.key;
				});
				absNodes = AbsNodes.join(ctx.options.persist.cookieDelimiter);
				local._data(local.cookiePrefix + SELECTEDABS, absNodes);
			} else {
				// beforeSelect can prevent the change - flag doesn't reflect the node.selected state
				local._appendKey(SELECTEDABS, node.key, node.selectedAbs);
			}
		}
		return res;
	}
...
*/
		...
		if( (node.isPresent() || node.isAbsent()) && !node.isActive() ) { classes[opts.selectedClass]  = true; }
