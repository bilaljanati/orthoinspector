...
$(document).ready(function() {
	...
	/* Determine size (nb of prots or nb of clusters) */
	function get_result_length(results) {
		if (Array.isArray(results)) {
			return results.length;
		}
		return results['clusters'].length + 1;
	}
	...
	function change_page(e) {
		...
		display_pagination(get_result_length(results), selected);
	}

	function display_pagination(nbresults, selected=1) {
		const pagesize = get_page_size();
		const nbpages = Math.ceil(nbresults/pagesize);
		console.log(nbresults);
		console.log(pagesize, nbpages);
	...
	function download_fasta() {
		...
		add_form_field(form, "filename", "go_"+get_go_term()+".fasta");
